...
-- =============================================
CREATE PROCEDURE [dbo].[uspAdhocFeePaymentHistoryGridSelect]
...
BEGIN TRY
 ...
 DECLARE @OrderBy_ASC_DESC NVARCHAR(20)=JSON_VALUE(@RequestModel, '$.getListModel.order[0].dir');
  ...
  SELECT COUNT(f.AdhocFeePaymentId)
  FROM AdhocFeePayment f
  INNER JOIN @FeePaymentTypeTable t
  ON f.PaymentTypeId = t.PaymentTypeId
  LEFT JOIN dbo.AdhocParticularMaster apm
  ON f.ParticularId = apm.AdhocParticularMasterId
  WHERE
       f.IsDeleted <> 1
   ...
   OR  LEN(@SearchText)>0 AND (f.TotalFee LIKE '%'+@SearchText+'%')
   OR  LEN(@SearchText)>0 AND (apm.Particular LIKE '%'+@SearchText+'%')
 ...
 ORDER BY
 CASE WHEN @OrderBy=0 THEN row_number() over(order by AdhocFeePaymentId asc) END DESC,
 CASE WHEN @OrderBy=1 THEN f.InvoiceNumber END DESC,
 CASE WHEN @OrderBy=2 THEN apm.Particular END DESC,
 CASE WHEN @OrderBy=3 THEN f.OnlineTransactionDateTime END DESC,
 CASE WHEN @OrderBy=4 THEN f.TotalFee END DESC,
 CASE WHEN @OrderBy=5 THEN t.PaymentTypeName END DESC,
 CASE WHEN @OrderBy=6 THEN f.ChequeDate END DESC,
 CASE WHEN @OrderBy=7 THEN IIF(f.PaymentTypeId = 2 OR f.PaymentTypeId = 3, IIF(f.IsChequeClear = 1, 'Yes', 'No'),'') END DESC,
 CASE WHEN @OrderBy=8 THEN f.OnlineTransactionId END DESC
...
ELSE
 ...
 ORDER BY
 CASE WHEN @OrderBy=0 THEN row_number() over(order by AdhocFeePaymentId asc) END ASC,
 CASE WHEN @OrderBy=1 THEN f.InvoiceNumber END ASC,
 CASE WHEN @OrderBy=2 THEN apm.Particular END DESC,
 CASE WHEN @OrderBy=3 THEN f.OnlineTransactionDateTime END ASC,
 CASE WHEN @OrderBy=4 THEN f.TotalFee END ASC,
 CASE WHEN @OrderBy=5 THEN t.PaymentTypeName END ASC,
 CASE WHEN @OrderBy=6 THEN f.ChequeDate END ASC,
 CASE WHEN @OrderBy=7 THEN IIF(f.PaymentTypeId = 2 OR f.PaymentTypeId = 3, IIF(f.IsChequeClear = 1, 'Yes', 'No'),'') END ASC,
 CASE WHEN @OrderBy=8 THEN f.OnlineTransactionId END ASC
 OFFSET @PageNumber ROWS
 FETCH NEXT @PageSize ROWS ONLY
 END
