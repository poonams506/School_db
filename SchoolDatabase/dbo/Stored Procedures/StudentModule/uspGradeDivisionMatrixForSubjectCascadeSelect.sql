...
-- =============================================
CREATE PROC uspGradeDivisionMatrixForSubjectCascadeSelect 
(@AcademicYearId int)
AS Begin 
SET 
...
FROM 
  ...
  LEFT  JOIN (SELECT DISTINCT GradeId,DivisionId FROM dbo.Subject WHERE AcademicYearId=@AcademicYearId AND IsDeleted <> 1) fgd 
