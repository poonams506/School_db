...
-- Description:  This stored procedure is used to get grade division mapping info details by association of Subject Mapping for cascading only
-- =============================================
CREATE PROC [dbo].[uspGradeDivisionMatrixForSubjectMappingCascadeSelect] 
(@AcademicYearId INT)
AS BEGIN 
SET 
...
FROM 
  ...
  LEFT  JOIN (SELECT DISTINCT GradeId,DivisionId FROM dbo.SubjectMapping WHERE AcademicYearId=@AcademicYearId AND IsDeleted <> 1) fgd 
