...
-- Description:  This stored procedure is used insert CabDriver profile
-- =============================================
CREATE PROCEDURE uspCabDriverUpsert (
  @CabDriverId BIGINT,
  @SchoolId SmallINT,
...
BEGIN TRY IF @CabDriverId > 0 BEGIN --Update Statement
SET @AppAccessOldMobileNo=(SELECT TOP 1 AppAccessMobileNo FROM CabDriver WHERE CabDriverId=@CabDriverId);
UPDATE 
  CabDriver 
SET
  [SchoolId] = @SchoolId,
...
WHERE 
  [CabDriverId] = @CabDriverId END ELSE BEGIN --INSERT Statement
  INSERT INTO CabDriver(
    [SchoolId],
    ...
    [BloodGroup], [ProfileImageUrl], [DrivingLicenceNumber], [ValidTill],
...
VALUES 
  (
    ...
    @TalukaName, @DistrictName, @StateName, @CountryName,
    @ZipCode, @Education, @BirthDate, 
    @BloodGroup, @ProfileImageUrl, @DrivingLicenceNumber, @ValidTill, @UserId, 
    @CurrentDateTime,
    @IsAppAccess, @AppAccessMobileNo, @AppAccessOneTimePassword
  ) 
   SET @CabDriverId = SCOPE_IDENTITY();
  ...
  IF  EXISTS(SELECT 1 FROM CabDriver WHERE AppAccessMobileNo=@AppAccessOldMobileNo)
  BEGIN
    SET @IsDifferentUserRoleExist=1;
  END
  EXEC uspUserUpsert @IsAppAccess,@AppAccessOldUserId,@IsDifferentUserRoleExist,@AppAccessMobileNo,@Upassword,@PasswordSalt,6,@UserId,@FirstName,@LastName,@CabDriverId 
