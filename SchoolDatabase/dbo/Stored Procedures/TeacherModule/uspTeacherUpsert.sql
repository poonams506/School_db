...
-- Description:  This stored procedure is used insert Teacher profile
-- =============================================
CREATE PROCEDURE uspTeacherUpsert (
  @TeacherId BIGINT, 
...
BEGIN TRY IF @TeacherId > 0 BEGIN --Update Statement
SET @AppAccessOldMobileNo=(SELECT TOP 1 AppAccessMobileNo FROM Teacher WHERE TeacherId=@TeacherId);
UPDATE 
  Teacher 
SET 
  [FirstName] = @FirstName, 
...
WHERE 
  [TeacherId] = @TeacherId END ELSE BEGIN --INSERT Statement
  INSERT INTO Teacher(
...
VALUES 
  ...
  END
  EXEC uspUserUpsert @IsAppAccess,@AppAccessOldUserId,@IsDifferentUserRoleExist,@AppAccessMobileNo,@Upassword,@PasswordSalt,3,@UserId,@FirstName,@LastName,@TeacherId
