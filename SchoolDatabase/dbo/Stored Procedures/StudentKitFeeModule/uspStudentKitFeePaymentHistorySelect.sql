...
-- =============================================
CREATE PROCEDURE [dbo].[uspStudentKitFeePaymentHistorySelect]
...
BEGIN TRY 
		...
		ON p.StudentId = s.StudentId AND p.AcademicYearId = m.AcademicYearId AND p.StudentKitFeePaymentId = @StudentKitFeePaymentId
		...
		INNER JOIN StudentKitFeePaymentDetails d
		ON d.FeeParticularId = f.FeeParticularId AND s.StudentId = d.StudentId AND d.AcademicYearId = @AcademicYearId AND d.StudentKitFeePaymentId = @StudentKitFeePaymentId
		WHERE 
		...
		AND f.IsPublished = 1 
		AND d.StudentKitFeePaymentId = @StudentKitFeePaymentId
