...
-- =============================================
CREATE PROCEDURE [dbo].[uspTransportFeePaymentHistoryGridSelect]
...
BEGIN TRY
 ...
 DECLARE @ConsumerId BIGINT = JSON_VALUE(@RequestModel, '$.consumerId');
 DECLARE @TransportConsumerStoppageMappingId INT = JSON_VALUE(@RequestModel, '$.transportConsumerStoppageMappingId');
 ...
 DECLARE @OrderBy_ASC_DESC NVARCHAR(20)=JSON_VALUE(@RequestModel, '$.getListModel.order[0].dir');
  ...
  SELECT COUNT(f.TransportFeePaymentId)
  ...
  WHERE
   ...
   AND f.ConsumerId = @ConsumerId
   AND f.TransportConsumerStoppageMappingId = @TransportConsumerStoppageMappingId
 ...
 ORDER BY
 CASE WHEN @OrderBy=0 THEN row_number() over(order by TransportFeePaymentId asc) END DESC,
...
ELSE
 ...
 ORDER BY
 CASE WHEN @OrderBy=0 THEN row_number() over(order by TransportFeePaymentId asc) END ASC,
