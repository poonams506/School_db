...
-- =============================================
CREATE  PROCEDURE [dbo].[uspTransportFeePaymentStudentGridSelect]
...
BEGIN TRY
 ...
 SELECT DISTINCT S.StudentId 
  FROM StudentGradeDivisionMapping M
	   ...
	   INNER JOIN TransportConsumerStoppageMapping FP
	   ON M.AcademicYearId = FP.AcademicYearId AND M.StudentId = FP.ConsumerId AND FP.RoleId = 5 AND FP.IsDeleted <> 1 and M.IsDeleted <> 1 AND M.AcademicYearId = @AcademicYearId
 ...
 INSERT INTO @temp3
  SELECT dbo.udfConsumerTransportTotalFee(@AcademicYearId, S.StudentId, 5) AS 'TotalFee',
     dbo.udfConsumerTransportDiscountedFee(@AcademicYearId, S.StudentId, 5) AS 'DiscountedFee',
     dbo.udfConsumerTransportPaidAmount(@AcademicYearId, S.StudentId, 5) AS 'PaidAmount',
     dbo.udfConsumerTransportOtherPaidAmount(@AcademicYearId, S.StudentId, 5) AS 'OtherPaidAmount',
     dbo.udfConsumerTransportDueAmount(@AcademicYearId, S.StudentId, 5) AS 'DueAmount',
     dbo.udfConsumerTransportChequeClearedAmount(@AcademicYearId, S.StudentId, 5) AS 'ChequeClearedAmount',
     dbo.udfConsumerTransportChequeUnClearAmount(@AcademicYearId, S.StudentId, 5) AS 'ChequeUnclearAmount',
 ...
 CASE WHEN @OrderBy=5 THEN temp.DiscountedFee END DESC,
 CASE WHEN @OrderBy=6 THEN temp.PaidAmount END DESC,
 CASE WHEN @OrderBy=7 THEN temp.DueAmount END DESC,
 CASE WHEN @OrderBy=8 THEN temp.ChequeClearedAmount END DESC,
 CASE WHEN @OrderBy=9 THEN temp.ChequeUnclearAmount END DESC
 OFFSET @PageNumber ROWS
 FETCH NEXT @PageSize ROWS ONLY
 END
...
ELSE
 ...
 CASE WHEN @OrderBy=5 THEN temp.DiscountedFee END ASC,
 CASE WHEN @OrderBy=6 THEN temp.PaidAmount END ASC,
 CASE WHEN @OrderBy=7 THEN temp.DueAmount END ASC,
 CASE WHEN @OrderBy=8 THEN temp.ChequeClearedAmount END ASC,
 CASE WHEN @OrderBy=9 THEN temp.ChequeUnclearAmount END ASC
 OFFSET @PageNumber ROWS
 FETCH NEXT @PageSize ROWS ONLY
  END
