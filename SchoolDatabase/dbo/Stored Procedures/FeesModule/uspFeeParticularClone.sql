...
-- =============================================
CREATE PROCEDURE uspFeeParticularClone
(
@AcademicYearId INT,
@FromClassId INT,
...
BEGIN TRY 
  ...
  WHERE sgdm.IsDeleted <> 1 AND sgdm.SchoolGradeDivisionMatrixId=@FromClassId AND sgdm.AcademicYearId = @AcademicYearId
    ...
    INSERT INTO dbo.FeeParticular(AcademicYearId, GradeId, DivisionId, ParticularName, Amount, IsDiscountApplicable,IsRTEApplicable, IsPublished, SortBy, CreatedBy, CreatedDate)
     SELECT @AcademicYearId, sgdm.GradeId, sgdm.DivisionId, f.ParticularName, f.Amount, f.IsDiscountApplicable, f.IsRTEApplicable, 0, f.SortBy, @UserId, @CurrentDateTime 
     FROM dbo.FeeParticular f CROSS JOIN 
     dbo.SchoolGradeDivisionMatrix sgdm  JOIN
	...
	INSERT INTO dbo.FeeParticularWavierMapping(AcademicYearId, GradeId, DivisionId, FeeWavierTypeId, CreatedBy, CreatedDate)
     ...
     FROM dbo.FeeParticularWavierMapping f CROSS JOIN 
     dbo.SchoolGradeDivisionMatrix sgdm  JOIN
     @ToClassId d ON sgdm.SchoolGradeDivisionMatrixId=d.Id
     WHERE f.GradeId=@FromGradeId AND f.DivisionId=@FromDivisionId AND
     f.AcademicYearId=@AcademicYearId AND f.IsDeleted <> 1;
