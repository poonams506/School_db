...
-- =============================================
CREATE PROC uspGradeDivisionMatrixForStudentKitFeesStructureCascadeSelect 
(@AcademicYearId INT)
AS BEGIN 
SET 
...
FROM 
  ...
  LEFT  JOIN (SELECT DISTINCT GradeId,DivisionId FROM dbo.StudentKitFeeParticular WHERE AcademicYearId=@AcademicYearId AND IsDeleted <> 1) fgd 
