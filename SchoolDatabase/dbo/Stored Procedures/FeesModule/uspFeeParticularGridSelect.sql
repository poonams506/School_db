...
-- =============================================
CREATE PROCEDURE [dbo].[uspFeeParticularGridSelect]
...
BEGIN TRY
 ...
 DECLARE @OrderBy_ASC_DESC NVARCHAR(20)=JSON_VALUE(@RequestModel, '$.getListModel.order[0].dir');
  ...
  FROM dbo.SchoolGradeDivisionMatrix M
     ...
     OUTER APPLY (SELECT TOP (1) FP.FeePaymentDetailId 
                    FROM dbo.FeePaymentDetails fp WHERE
 ...
 SELECT 
     ...
     CASE WHEN F.IsPublished = 1 THEN 'Published'
          ...
          ELSE 'Not-Created'
          END AS Status,
    CASE WHEN fp.FeePaymentDetailId IS NOT NULL THEN 1
   ...
   FROM dbo.SchoolGradeDivisionMatrix M
      ...
      ON M.DivisionId = D.DivisionId
	 OUTER APPLY (SELECT TOP (1) FeeParticularId,IsPublished 
                   FROM dbo.FeeParticular  WHERE GradeId=M.GradeId  
