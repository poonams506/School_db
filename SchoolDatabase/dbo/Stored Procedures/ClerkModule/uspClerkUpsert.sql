...
-- Description:  This stored procedure is used insert Clerk profile
-- =============================================
CREATE PROCEDURE uspClerkUpsert (
  @ClerkId BIGINT,
...
BEGIN TRY IF @ClerkId > 0 BEGIN --Update Statement
SET @AppAccessOldMobileNo=(SELECT TOP 1 AppAccessMobileNo FROM Clerk WHERE ClerkId=@ClerkId);
UPDATE 
  Clerk 
SET
  [FirstName] = @FirstName, 
...
WHERE 
  [ClerkId] = @ClerkId END ELSE BEGIN --INSERT Statement
  INSERT INTO Clerk(
...
VALUES 
  ...
  IF  EXISTS(SELECT 1 FROM Clerk WHERE AppAccessMobileNo=@AppAccessOldMobileNo)
  BEGIN
    SET @IsDifferentUserRoleExist=1;
  END 
  EXEC uspUserUpsert @IsAppAccess,@AppAccessOldUserId,@IsDifferentUserRoleExist,@AppAccessMobileNo,@Upassword,@PasswordSalt,4,@UserId,@FirstName,@LastName,@ClerkId 
